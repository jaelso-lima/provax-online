...
export async function generateEditalPdf(resultado: AnalysisResult, fileName: string) {
  ...
  doc.setFontSize(14);
  doc.setFont("helvetica", "bold");
  doc.text("Guia de Estudo Completo", margin, y + 2);
  y = 40;
  ...
  resultado.materias?.forEach((mat, idx) => {
    ...
    if (mat.conteudos_principais?.length) {
      checkPage(10);
      doc.setFont("helvetica", "bold");
      addWrappedText(`Conteúdos do Edital (${mat.conteudos_principais.length} itens)`, margin, 10, [30, 30, 30]);
      y += 1;
      doc.setFont("helvetica", "normal");
    ...
    // RESUMO DETALHADO - heart of the guide
    if (mat.resumo_detalhado) {
      ...
      resumoLines.forEach(line => {
        if (line.trim()) {
          addWrappedText(line, margin, 9, [40, 40, 40]);
          y += 1;
        } else {
          y += 3;
        }
    ...
    if (mat.macetes?.length) {
      checkPage(10);
      doc.setFont("helvetica", "bold");
      addWrappedText("🧠 MACETES DE MEMORIZAÇÃO", margin, 10, [128, 0, 255]);
      y += 1;
      doc.setFont("helvetica", "normal");
    ...
    if (mat.exemplos?.length) {
      checkPage(10);
      doc.setFont("helvetica", "bold");
      addWrappedText("Exemplos de Questões", margin, 10, [30, 30, 30]);
      y += 1;
      doc.setFont("helvetica", "normal");
  ...
  doc.save(`guia_estudo_${safeName}.pdf`);
}
